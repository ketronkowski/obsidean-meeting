...
${transcriptContent}`;
	...
	private async combineSummaries(content: string, transcriptSummary: string): Promise<string | null> {
		...
		const isDisclaimer = copilotSummary.length < 100 || 
		                     copilotSummary.toLowerCase().includes('disclaimer') ||
		                     copilotSummary.toLowerCase().includes('generated by ai') ||
		                     copilotSummary.toLowerCase().includes('may not be accurate');
